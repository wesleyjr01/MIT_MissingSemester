...
This way you do not need to reenter your password every time. Nevertheless, the private key (often `~/.ssh/id_rsa` and more recently `~/.ssh/id_ed25519`) is effectively your password, so treat it like so.
...
If you have ever configured pushing to GitHub using SSH keys, then you have probably done the steps outlined here and [have](https://help.github.com/en/github/authenticating-to-github/connecting-to-github-with-ssh) a valid key pair already. To check if you have a passphrase and validate it you can run `ssh-keygen -y -f /path/to/key`.
...
* Performance (some newer terminals like [Alacritty](https://github.com/alacritty/alacritty) or [kitty](https://sw.kovidgoyal.net/kitty/) offer GPU acceleration).